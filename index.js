...
const console = require("prefix-logger")("mc-font-extractor")
const fetch = require("node-fetch");
const JSZip = require("jszip");

const extractCharMap = async (version) => {
  ...
  console.log("downloading jar...", "mc-font-extractor.init");

  const jar = await fetch(versions[version].url).then((res) =>
    res.arrayBuffer()
  );

  console.log("loading jar...", "mc-font-extractor.init");
  ...
  if (versions[version].pack_version >= 4) {
    console.log("using pack version 4/5/6", "mc-font-extractor.init");
...
module.exports = { extractCharMap };

const opentype = require("opentype.js");
const UPNG = require("upng-js");

const makeGlyphs = async () => {
  const { providers, zip } = await extractCharMap("1.16.5");
  ...
  for (var i = 0; i < providers.length; i++) {
    ...
    if (imagepath) {
      console.log(
        "Reading provider " + providers[i].file,
        "mc-font-extractor.makeGlyphs"
      );
      ...
      if (commonMultiple != GCF) {
        ...
        console.log(
          "Scaling glyphs by " + GCF / commonMultiple,
          "mc-font-extractor.makeGlyphs"
        );
        commonMultiple = GCF;
      }
      ...
      for (var j = 0; j < charsPerRow * charsPerCol; j++) {
        ...
        if (!glyphs[currentcodepoint]) {
          ...
          console.log(
            `Skipped duplicate glyph (${currentcodepoint}): ${String.fromCodePoint(currentcodepoint)}`,
            "mc-font-extractor.makeGlyphs"
          );
        }
      }
      ...
      console.log(
        "Skipping provider " + providers[i].type,
        "mc-font-extractor.makeGlyphs"
      );
    }
  }
  ...
  console.log(`Found ${totalGlyphCount} glyphs!`, "mc-font-extractor");

  require("fs").writeFileSync("font.otf", Buffer.from(finalFont.toArrayBuffer()));
  ...
  for (g in glyphs) {
    ...
    var result = "\\u" + "0000".substring(0, 4 - unicodeHex.length) + unicodeHex;
    a.push({
      uid: result,
      id: glyphs[g].unicode,
      val,
  ...
  //new opentype.Glyph().getBoundingBox().x2

  //new opentype.Glyph().getMetrics().

  require("fs").writeFileSync("data.json", JSON.stringify(a));

  require("fs").writeFileSync("data_chars.txt", a.map(v => {return v.val}).join("").toString())
